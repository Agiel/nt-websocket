...
const TEAMS = ["None", "Spectator", "Jinrai", "NSF"];

const CLASSES = ["Recon", "Assault", "Support", "VIP", "Juggernaut"];
  ...
  try {
    const res =
      process.env.NODE_ENV === "development"
        ? await fetch(
            "http://" + window.location.hostname + ":3000/avatar/" + steamIds
          )
...
function getAvatarURL(player) {
  if (isNaN(parseInt(player.steamId))) return;
  ...
  switch (type) {
    ...
    case "I": {
      ...
      const match = serverName.match(
        /(bonahnsa|sweaty|baux|bulletnauts|eu|agiel)/i
      );
      if (match != null) {
    ...
    case "J": {
      ...
      player.health = HEALTH_FOR_CLASS[player.class];
      break;
    }
    // * K: Player died
    case "K": {
    ...
    case "R":
      ...
      store.players.forEach((player) => {
        player.roundKills = 0;
        // Currently R happens after S, but we shouldn't need the spawning state in NT;RE anyway.
    ...
    case "Z": {
      ...
      console.log(
        "Z -- Stage: " + stage + " && Team: " + team + " && Map: " + map
      );
...
async function connectManager(serverId) {
  try {
    const localWs =
      process.env.NODE_ENV === "development"
        ? new WebSocket(
            `ws://${window.location.hostname}:3000/state/${serverId}`
          )
  ...
  try {
    const res =
      process.env.NODE_ENV === "development"
        ? await fetch(
            `http://${window.location.hostname}:3000/state/${serverId}`
          )
