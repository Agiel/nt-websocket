  ...
  <div class="panel-container flex">
    ...
    >
      ...
      <div class="info flex">
        ...
        <div class="stats">
          <div v-if="data.roundKills" class="kills">
            {{ "x" + data.roundKills }}
          </div>
        </div>
        ...
        <div class="weapons-container align" v-if="data.isAlive">
          <img
            ...
            class="weapon-icon"
            :class="{
              [getWeaponType(weapon)]: true,
              active: data.activeWeapon === weapon,
            }"
...
import { getWeaponType, HEALTH_FOR_CLASS } from "../store";
...
const healthPercent = computed(() => {
  if (props.data.isAlive) {
    return (100 * props.data.health) / HEALTH_FOR_CLASS[props.data.class];
  } else {
    return "";
  }
});
...
const equippedWeapons = computed(() => {
  let weapons = [...props.data.equippedWeapons.values()];
  if (!weapons.includes(props.data.activeWeapon)) {
    weapons.push(props.data.activeWeapon);
  }
  weapons = weapons
    .filter((w) => w !== "ghost")
    .toSorted((a, b) => getWeaponType(b).localeCompare(getWeaponType(a)));
  return weapons;
});
</script>
...
.health {
  text-shadow: 0 0 4px black, -1px -1px 1px black, 0 -1px 1px black,
    1px -1px 1px black, -1px 0 1px black, 1px 0 1px black, -1px 1px 1px black,
